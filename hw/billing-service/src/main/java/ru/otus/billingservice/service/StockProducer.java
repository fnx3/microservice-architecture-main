...
public class StockProducer {
    ...
    private void sendWithRetry(OrderMessage message, int maxRetries) {
        ListenableFuture<SendResult<String, OrderMessage>> future = kafkaTemplate.send(STOCK_TOPIC, message);

        future.addCallback(new ListenableFutureCallback<>() {
            @Override
            public void onFailure(Throwable ex) {
                log.error("Order message with id '{}' failed to push on stock topic: {}", message.getOrderId(), ex.getMessage());
                if (maxRetries > 0) {
            ...
            public void onSuccess(SendResult<String, OrderMessage> result) {
                log.info("Order message with id '{}' successfully pushed on stock topic", message.getOrderId());
            }
        });
    }
}
